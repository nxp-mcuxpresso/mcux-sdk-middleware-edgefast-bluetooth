...
static void pac_notify_loc(struct k_work *work)
{
	...
	err = get_pac_loc(NULL, dir, (enum bt_audio_location *)&location);
	if (err != 0) {
...
struct k_work_delayable available_contexts_work;

int bt_pacs_available_contexts_changed(void)
{
	int err;
	static bool available_contexts_work_init = false;

	if(!available_contexts_work_init)
	{
		k_work_init_delayable(&available_contexts_work, available_contexts_notify);
	}
