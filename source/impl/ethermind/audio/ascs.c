...
static void ascs_ep_unbind_audio_iso(struct bt_audio_ep *ep)
{
	...
	if (audio_iso != NULL) {
		...
		__ASSERT(stream != NULL, "Stream was NULL");
				  ...
				  uint8_t reason)
{
	...
	case -ENOBUFS:
	case -ENOMEM:
		ascs_cp_rsp_add(id, op, BT_ASCS_RSP_NO_MEM,
				       BT_ASCS_REASON_NONE);
                break;
	case -EINVAL:
		switch (op) {
		...
		case BT_ASCS_METADATA_OP:
			ascs_cp_rsp_add(id, op,
                        ...
                        break;
		default:
			ascs_cp_rsp_add(id, op, BT_ASCS_RSP_UNSPECIFIED,
					       BT_ASCS_REASON_NONE);
                        break;
		}
                break;
	case -ENOTSUP:
		...
		default:
			ascs_cp_rsp_add(id, op,
                ...
                break;
	case -EBADMSG:
		ascs_cp_rsp_add(id, op, BT_ASCS_RSP_INVALID_ASE_STATE,
					       BT_ASCS_REASON_NONE);
                break;
	case -EACCES:
		switch (op) {
...
{
	const enum bt_audio_dir dir = (enum bt_audio_dir)stream->ep->dir;
...
static bool ascs_parse_metadata(struct bt_data *data, void *user_data)
{
	...
	if (IS_ENABLED(CONFIG_BT_CAP_ACCEPTOR)) {
		if (data_type == BT_AUDIO_METADATA_TYPE_STREAM_CONTEXT) {
			...
			if (!bt_pacs_context_available((enum bt_audio_dir)ep->dir, context)) {
			     ...
			     uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	if (!ascs) {
		BT_ERR("Unable to get ASCS session");
		len = (uint16_t)BT_GATT_ERR(BT_ATT_ERR_UNLIKELY);
