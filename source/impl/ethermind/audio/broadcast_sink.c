		      ...
		      struct bt_le_per_adv_sync_synced_info *info)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&sink_cbs, listener, _node, struct bt_audio_broadcast_sink_cb) {
...
static void broadcast_sink_cleanup_streams(struct bt_audio_broadcast_sink *sink)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&sink->streams, stream, next, _node, struct bt_audio_stream) {
		if (stream->ep != NULL) {
