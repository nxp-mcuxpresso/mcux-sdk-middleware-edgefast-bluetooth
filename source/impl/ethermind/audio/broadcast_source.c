...
static void broadcast_source_cleanup(struct bt_audio_broadcast_source *source)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&source->streams, stream, next, _node, struct bt_audio_stream) {
				       ...
				       struct bt_codec_qos *qos)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&source->streams, stream, _node, struct bt_audio_stream) {
