...
#define LOG_ENABLE IS_ENABLED(CONFIG_BT_AUDIO_DEBUG_UNICAST_CLIENT)
						       ...
						       uint16_t length)
{
	...
	if (unicast_client_cbs != NULL &&
		...
		unicast_client_cbs->available_contexts(conn, (enum bt_audio_context)context.snk,
						       (enum bt_audio_context)context.src);
	}
