			       ...
			       const struct bt_codec_qos *qos)
{
	...
	cis_count = 0;
	SYS_SLIST_FOR_EACH_CONTAINER(&group->streams, stream, _node, struct bt_audio_stream) {
		if (stream->iso != NULL) {
				       ...
				       size_t num_param)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&unicast_group->streams, tmp_stream, _node, struct bt_audio_stream) {
		total_stream_cnt++;
	}
...
int bt_audio_unicast_group_delete(struct bt_audio_unicast_group *unicast_group)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&unicast_group->streams, stream, tmp, _node, struct bt_audio_stream) {
