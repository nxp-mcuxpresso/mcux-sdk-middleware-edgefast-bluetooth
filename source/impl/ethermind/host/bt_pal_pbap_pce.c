...
/* PBAP function name length (unicode)*/
#define BT_FUNC_PULL_PHONEBOOK_NAME_MAX_LEN      40U /* SIM1/telecom/xxx.vcf*/
#define BT_FUNC_SET_PHONEBOOK_PATH_NAME_MAX_LEN  14U /* telecom */
#define BT_FUNC_PULL_VACARD_LISTING_NAME_MAX_LEN 14U /* xxx.vcf */
#define BT_FUNC_PULL_VACARD_ENTRY_NAME_MAX_LEN   82U /* X-BT-UID:XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX */

NET_BUF_POOL_FIXED_DEFINE(pbap_pool, PBAP_PCE_MAX_ENTITY, CONFIG_BT_PBAP_PCE_MAX_PKT_LEN, NULL);
    ...
    switch (tag_id)
    {
        ...
        case BT_PBAP_TAG_ID_SEARCH_VALUE:
            BT_PBAP_ADD_PARAMS_SEARCH_VALUE(buf, appl_params->search_value.value,
                                            (uint8_t)BT_str_len(appl_params->search_value.value));
            break;
        ...
        case BT_PBAP_TAG_ID_PHONE_BOOK_SIZE:
            BT_PBAP_ADD_PARAMS_PHONE_BOOK_SIZET(buf, appl_params->phonebook_size);
            break;
                                 ...
                                 enum bt_obex_req_flags flag)
{
    ...
    if (flag & BT_OBEX_REQ_START)
    {
        /* name*/
        hi      = OBEX_HDR_NAME;
        str_len = (uint16_t)BT_str_len(name);
...
int bt_pbap_pce_pull_phonebook(struct bt_pbap_pce *pbap_pce, struct net_buf *buf, char *name, bool wait, enum bt_obex_req_flags flag)
{
    ...
    if ((flag & BT_OBEX_REQ_START))
    {
        if (name == NULL || BT_str_len(name) > BT_FUNC_PULL_PHONEBOOK_NAME_MAX_LEN)
        {
            return -EINVAL;
        }
        name_header.value  = (UCHAR *)name;
        name_header.length = (uint16_t)BT_str_len(name);
    }
...
static void bt_pbap_add_pbap_setPhonebookPath_hdr(struct net_buf *buf, char *name)
{
    ...
    uint16_t length;
    uint16_t packet_len;
    length = (uint16_t)BT_str_len(name);
    ...
    switch (event_type)
    {
        case PBAP_PCE_CONNECT_CFM:
            ...
            if (BT_PBAP_UNAUTH_RSP == event_result)
            {
                ...
                if (pbap_pce->auth != NULL && pbap_pce->auth->pin_code != NULL &&
                    BT_str_len(pbap_pce->auth->pin_code) > 0)
                {
