...
static shell_status_t cmd_mce_register(shell_handle_t shell, int32_t argc, char *argv[])
{
...
static shell_status_t cmd_mce_unregister(shell_handle_t shell, int32_t argc, char *argv[])
{
    shell_map_deinit();
...
static shell_status_t cmd_mce_mns_register(shell_handle_t shell, int32_t argc, char *argv[])
{
    bt_map_mce_mns_register(&mce_mns_cb);
    return kStatus_SHELL_Success;
}

static shell_status_t cmd_mce_mns_unregister(shell_handle_t shell, int32_t argc, char *argv[])
{
    bt_map_mce_mns_unregister();
    return kStatus_SHELL_Success;
}

static shell_status_t cmd_mce_connect(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err = bt_sdp_discover(default_conn, &discov_map_mse);

    if (err)
    {
        shell_error(shell, "SDP discovery failed: %d", err);
...
static shell_status_t cmd_mce_disconnect(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err = bt_map_mce_disconnect(app_mce_instance.mce_mas);

    if (err < 0)
    {
        shell_error(shell, "disconnect failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
    return kStatus_SHELL_Success;
}

static shell_status_t cmd_mce_mns_disconnect(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err = bt_map_mce_mns_disconnect(app_mce_instance.mce_mns);

    if (err < 0)
    {
        shell_error(shell, "disconnect MNS failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
    return kStatus_SHELL_Success;
}

static shell_status_t cmd_abort(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err = bt_map_mce_abort(app_mce_instance.mce_mas);

    if (err == -EINPROGRESS)
    {
        shell_print(shell, "abort is pending");
    }
    else if (err < 0)
    {
        shell_error(shell, "abort failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
    return kStatus_SHELL_Success;
}

static shell_status_t cmd_get_folder_list(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    struct net_buf *buf;
    uint32_t index;
    ...
    {
        net_buf_unref(buf);
        shell_error(shell, "get folder listing failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static shell_status_t cmd_set_folder(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    uint32_t index;
    ...
    if (err != 0)
    {
        shell_error(shell, "set folder failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static int cmd_get_msg_list_param(int32_t argc, char *argv[], struct net_buf *buf, uint32_t *param_len)
{
    uint32_t index;
    uint32_t tag_u32;
    *param_len = 0;
...
static shell_status_t cmd_get_msg_list(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    struct net_buf *buf;
    uint16_t max_body_len;
    ...
    {
        shell_error(shell, "parameters wrong or too long");
        return kStatus_SHELL_Error;
    }
    buf = net_buf_alloc(&mce_mas_tx_pool, osaWaitForever_c);
    ...
    {
        net_buf_unref(buf);
        shell_error(shell, "get msg list failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static shell_status_t cmd_get_msg(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    struct net_buf *buf;
    uint32_t index;
    ...
    {
        net_buf_unref(buf);
        shell_error(shell, "get msg failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static int cmd_set_msg_status_param(int32_t argc, char *argv[], struct net_buf *buf, uint32_t *param_len, char *name)
{
    uint32_t index;
    uint32_t tag_u32;
    *param_len = 0;
...
static shell_status_t cmd_set_msg_status(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    struct net_buf *buf;
    uint16_t max_body_len;
    ...
    {
        net_buf_unref(buf);
        shell_error(shell, "set msg status failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static int cmd_push_msg_param(int32_t argc, char *argv[], struct net_buf *buf, uint32_t *param_len)
{
    uint32_t index;
    uint32_t tag_u32;
    *param_len = 0;
...
static shell_status_t cmd_push_msg(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    struct net_buf *buf;
    ...
    {
        net_buf_unref(buf);
        shell_error(shell, "push msg failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
    else
    {
...
static shell_status_t cmd_set_ntf_reg(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    struct net_buf *buf;
    uint32_t index;
    ...
    {
        net_buf_unref(buf);
        shell_error(shell, "set notification registration failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static shell_status_t cmd_update_inbox(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    ...
    if (err != 0)
    {
        shell_error(shell, "update inbox failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static shell_status_t cmd_get_mas_inst_info(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    struct net_buf *buf;
    uint32_t index;
    ...
    {
        net_buf_unref(buf);
        shell_error(shell, "get MAS instance infomation failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static int cmd_set_owner_status_param(int32_t argc, char *argv[], struct net_buf *buf, uint32_t *param_len)
{
    uint32_t index;
    uint32_t tag_u32;
    *param_len = 0;
...
static shell_status_t cmd_set_owner_status(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    struct net_buf *buf;
    uint16_t max_body_len;
    ...
    {
        net_buf_unref(buf);
        shell_error(shell, "set owner status failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static shell_status_t cmd_get_owner_status(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    struct net_buf *buf;
    uint32_t index;
    ...
    {
        net_buf_unref(buf);
        shell_error(shell, "get owner status failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static int cmd_get_convo_list_param(int32_t argc, char *argv[], struct net_buf *buf, uint32_t *param_len)
{
    uint32_t index;
    uint32_t tag_u32;
    *param_len = 0;
...
static shell_status_t cmd_get_convo_list(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    struct net_buf *buf;
    uint16_t max_body_len;
    ...
    {
        net_buf_unref(buf);
        shell_error(shell, "get conversation listing failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static shell_status_t cmd_set_ntf_filter(shell_handle_t shell, int32_t argc, char *argv[])
{
    int err;
    struct net_buf *buf;
    uint32_t index;
    ...
    {
        net_buf_unref(buf);
        shell_error(shell, "set notification filter failed Reason %d", err);
        return kStatus_SHELL_Error;
    }
...
static shell_status_t cmd_mse_register(shell_handle_t shell, int32_t argc, char *argv[])
{
    struct net_buf *buf = NULL;
    struct bt_hci_cp_write_class_of_device *cp;
    int err;
...
static shell_status_t cmd_mse_unregister(shell_handle_t shell, int32_t argc, char *argv[])
{
...
static shell_status_t cmd_mse_mns_register(shell_handle_t shell, int32_t argc, char *argv[])
{
    bt_map_mse_mns_register(&mse_mns_cb);
    return kStatus_SHELL_Success;
}

static shell_status_t cmd_mse_mns_unregister(shell_handle_t shell, int32_t argc, char *argv[])
{
    bt_map_mse_mns_unregister();
    return kStatus_SHELL_Success;
}

static shell_status_t cmd_mse_disconnect(shell_handle_t shell, int32_t argc, char *argv[])
{
...
static shell_status_t cmd_mse_mns_disconnect(shell_handle_t shell, int32_t argc, char *argv[])
{
...
static shell_status_t cmd_send_event(shell_handle_t shell, int32_t argc, char *argv[])
{
    struct net_buf *buf;
    uint16_t max_pkt_len;
    uint16_t actual;
...
#if (defined(CONFIG_BT_MAP_MCE) && ((CONFIG_BT_MAP_MCE) > 0U))
    SHELL_CMD_ARG(mce, mce_cmds, HELP_NONE, NULL, 1, 0),
#endif
#if (defined(CONFIG_BT_MAP_MSE) && ((CONFIG_BT_MAP_MSE) > 0U))
    SHELL_CMD_ARG(mse, mse_cmds, HELP_NONE, NULL, 1, 0),
#endif
	SHELL_SUBCMD_SET_END
);

static shell_status_t cmd_map(shell_handle_t shell, int32_t argc, char **argv)
{
    if (argc == 1)
    {
        shell_help(shell);
...
SHELL_CMD_REGISTER(map, map_cmds, "Bluetooth MAP shell commands", cmd_map, 1, 1);
