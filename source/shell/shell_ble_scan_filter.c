...
static shell_status_t shell_le_is_dev_in_adv_list(bt_addr_le_t *dev_addr, uint8_t event_type)
{
    uint32_t  index = 0U;
    int       retval;
    ...
    return (shell_status_t)retval;
}

static shell_status_t shell_le_find_free_adv_list_inst(uint32_t *free_index)
{
    uint32_t  index = 0U;
    int       retval;
    bt_addr_le_t * t_addr;
    ...
    if (true != shell_le_peer_adv_report_list_full)
    {
        ...
        *(free_index) = index;
        ...
        if (kStatus_SHELL_Success != retval)
        {
...
shell_status_t shell_le_init_adv_list(void)
{
...
shell_status_t shell_le_scan_filter_handler(const struct bt_le_scan_recv_info *info)
{
    uint32_t index = 0;
     /* Validate if this device already exists in the filtering list */
    if (kStatus_SHELL_Success != shell_le_is_dev_in_adv_list((bt_addr_le_t *)info->addr, info->adv_type))
    {
        /* Device dosen't present in the filtering list, add the device to the filtering list */
        if (kStatus_SHELL_Success == shell_le_find_free_adv_list_inst(&index))
        {
            /* Save the Event type */
